...
def _run_scheduler(cfg: Config, dry_run: bool) -> int:
	cache_dir = Path(cfg.paths.cache_dir)
	ensure_dir(cache_dir)
	text = fetch_tles(cache_dir)
	triples = parse_tles(text)
	...
	passes = find_passes(targets, qth, cfg.lookahead_hours, cfg.min_elevation_deg)
	if not passes:
		print("No passes within lookahead window.")
		return 0
	
	scheduler = PassScheduler(cfg)
	scheduler.schedule_passes(passes, dry_run=dry_run)
	return 0
...
def main(argv: Optional[list[str]] = None) -> int:
	...
	if args.command == "list-passes":
		...
		return _run_scheduler(cfg, getattr(args, "dry_run", False))
	else:
		parser.print_help()
		return 0
